...
class UserFeedbackService {
  ...
  getFeedbackOptionsByCategory(category: string): FeedbackOption[] {
    return this.feedbackOptions.filter(option => option.category === category);
  }
  ...
  async applyUserFeedback(
    ...
    try {
      ...
      const selectedPrompts = feedback.selectedOptions
        .map(optionId => this.feedbackOptions.find(opt => opt.id === optionId))
...
`;
      ...
      // 적용된 변경사항 분석
      const changesApplied = await this.analyzeChanges(originalContent, revisedContent, feedback);

      // 개선 점수 계산
      const improvementScore = await this.calculateImprovementScore(originalContent, revisedContent);

      // 최종 품질 점수 계산
      const finalQualityScore = await this.calculateFinalQuality(revisedContent, contentData);
  ...
  private async analyzeChanges(original: string, revised: string, feedback: UserFeedback): Promise<string[]> {
  ...
  private async calculateImprovementScore(original: string, revised: string): Promise<number> {
    try {
      const systemPrompt = `
...
`;
  ...
  private async calculateFinalQuality(content: string, contentData: OrganizedContent): Promise<number> {
    try {
      const systemPrompt = `
