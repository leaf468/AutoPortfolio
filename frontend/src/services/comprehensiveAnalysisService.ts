...
export function extractCoreActivity(content: string): string {
  ...
  const skipPatterns = [
    // 회사 설명
    /^.{0,30}(회사|기업|대기업|현대|삼성|LG|카카오|네이버).*?(위해|통해|에서는|이며|입니다|있습니다)/,
    /세계적으로\s*유명|선두|최고|글로벌|리딩/,

    // 지원동기/다짐
    /기여하고\s*싶|기여하겠|투입할\s*것|열정과\s*지식|열망|바람|희망|목표|다짐|느껴서|매력을\s*느/,

    // 학습/능력 향상 (활동 아님)
    /이해와.*?능력을\s*키|능력을.*?발전|역량을.*?강화|지식과\s*경험\s*습득|학습하면서|배우|깨달|느끼|알게/,
    /중요성을|이해하고|습득하|발전시켰|향상시|키울\s*수\s*있었/,

    // 추상적 표현
    /에\s*대한\s*(지식|이해|경험).*?(보유|습득|키|갖)/,
    /을\s*통해.*?(능력|역량|이해).*?키|발전/,
