...
export interface BoosterQuestion {
  ...
  expectedAnswer: 'short_text' | 'long_text' | 'number' | 'date' | 'url' | 'file';
...
class InteractiveBooster {
  async generateQuestions(organizedContent: OrganizedContent): Promise<BoosterQuestion[]> {
    const systemPrompt = `
...
`;
  ...
  async createBoosterSession(organizedContent: OrganizedContent): Promise<BoosterSession> {
  ...
  async processAnswer(session: BoosterSession, questionId: string, answer: any): Promise<BoosterSession> {
    ...
    session.progress = Math.round((answeredCount / session.questions.length) * 100);
  ...
  async generateBoostResult(originalContent: OrganizedContent, session: BoosterSession): Promise<BoostResult> {
    const systemPrompt = `
...
` 
  ...
  async getNextQuestion(session: BoosterSession): Promise<BoosterQuestion | null> {
  ...
  getSessionStats(session: BoosterSession) {
    ...
    const highPriorityAnswered = Object.keys(session.answers).filter(id => {
      const question = session.questions.find(q => q.id === id);
      return question?.priority === 'high';
    }).length;
    const highPriorityTotal = session.questions.filter(q => q.priority === 'high').length;
    ...
    return {
      ...
      highPriorityTotal,
      estimatedTimeRemaining: Math.max(0, (totalQuestions - answeredQuestions) * 2), // 2분 per question
      completedCategories: Array.from(new Set(session.completedFields.map(field => field.split('[')[0])))
    };
  }
}
