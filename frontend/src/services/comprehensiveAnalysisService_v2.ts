...
export interface ActivityPattern {
  ...
  examples: string[]; // 실제 DB 데이터 (내부 처리용)
  anonymizedExamples?: string[]; // OpenAI로 익명화된 예시
  insight: string;
}

/**
 * OpenAI를 사용하여 활동 예시를 익명화 (배치 처리)
 */
...
# 역할
AI 분석 기반으로 익명화된 활동 예시를 생성하세요.

# 입력 데이터
직무: ${position}

다음은 ${position} 직무 분석 데이터입니다:
...
- 참고 데이터의 **패턴과 특징**은 유지하되, **개인정보나 구체적인 내용**은 일반화/익명화
- 예시는 현실적이고 구체적이어야 함
- 직무에 맞는 전문적인 표현 사용
...
/**
 * 전체 데이터에서 특정 직무의 종합 통계 분석 (Synthetic Data 사용)
 * @param position - 직무명
 * @param skipAnonymization - true면 익명화 처리를 건너뜀
 */
export async function getComprehensiveStats(position: string, skipAnonymization: boolean = false): Promise<ComprehensiveStats> {
  try {
    ...
    relevantApplicants.forEach(app => {
      (app.activities || []).forEach((act, idx) => {
        allActivities.push({
          id: app.id * 1000 + idx,
          cover_letter_id: app.id,
    ...
    // 활동 패턴 분석
    const activityPatterns = analyzeActivityPatterns(allActivities, relevantApplicants.length);

    // OpenAI를 통해 활동 예시 익명화
    ...
    const stats: ComprehensiveStats = {
      position,
      totalApplicants: relevantApplicants.length,
      avgGpa: calculateAvgGpa(relevantApplicants),
      gpaDistribution: calculateGpaDistribution(relevantApplicants),
      topUniversities: extractTopUniversities(relevantApplicants),
      topMajors: extractTopMajors(relevantApplicants),
      avgToeic: calculateAvgToeic(relevantApplicants),
      toeicDistribution: calculateToeicDistribution(relevantApplicants),
      commonActivities: finalActivityPatterns,
      topCertificates: extractTopCertificates(relevantApplicants),
      activityEngagement: calculateActivityEngagement(allActivities, relevantApplicants),
      topSkills: extractTopSkills(allActivities, relevantApplicants.length),
      insights: generateInsights(relevantApplicants, allActivities),
      recommendations: generateRecommendations(relevantApplicants, allActivities, position),
    };
...
function calculateAvgGpa(applicants: SyntheticApplicant[]): number {
  const gpas: number[] = [];
...
function calculateGpaDistribution(applicants: SyntheticApplicant[]): { range: string; percentage: number }[] {
  const ranges = [
...
function extractTopUniversities(applicants: SyntheticApplicant[]): { name: string; count: number }[] {
...
function extractTopMajors(applicants: SyntheticApplicant[]): { name: string; count: number }[] {
...
function calculateAvgToeic(applicants: SyntheticApplicant[]): number {
  const toeics: number[] = applicants
...
function calculateToeicDistribution(applicants: SyntheticApplicant[]): { range: string; percentage: number }[] {
  const ranges = [
...
function generateExamplesFromDB(
  ...
  for (const act of allActivities) {
    if (relatedActivities.length >= count * 3) break;
...
function analyzeActivityPatterns(activities: Activity[], totalApplicants: number): ActivityPattern[] {
  ...
  const results = Array.from(activityMap.entries())
    .filter(([keyword, data]) => data.personCount.size >= 1)
    .map(([keyword, data]) => {
      const rawPercentage = (data.personCount.size / totalApplicants) * 100;
      const percentage = Math.min(Math.round(rawPercentage * 10) / 10, 100);
...
function extractTopCertificates(applicants: SyntheticApplicant[]): { name: string; percentage: number; count: number }[] {
  ...
  const noCertPercentage = (noCertCount / applicants.length) * 100;
...
function calculateActivityEngagement(
  ...
  const avgActivityCount = counts.length > 0 ? counts.reduce((a, b) => a + b, 0) / applicants.length : 0;
  ...
  const distribution = [
    ...
    { range: '0개', count: applicants.length - counts.length },
  ].map(item => ({
    range: item.range,
    percentage: (item.count / applicants.length) * 100,
  }));
...
function generateRecommendations(
  ...
  // 학점 기준 추천
  const avgGpa = calculateAvgGpa(applicants);
  if (avgGpa >= 4.0) {
  ...
  // 어학 점수 추천
  const avgToeic = calculateAvgToeic(applicants);
  if (avgToeic >= 900) {
  ...
  if (avgActivityCount < 3) {
    recommendations.push('평균 3개 이상의 활동을 작성하는 것이 좋습니다. 다양한 경험을 추가로 작성해보세요.');
  ...
  // 자격증 추천
  const certs = extractTopCertificates(applicants);
...
function generateInsights(applicants: SyntheticApplicant[], activities: Activity[]): string[] {
  ...
  const avgGpa = calculateAvgGpa(applicants);
  if (avgGpa > 0) {
    insights.push(`평균 학점은 ${avgGpa.toFixed(2)}/4.5로, ${avgGpa >= 4.0 ? '매우 높은' : avgGpa >= 3.5 ? '높은' : '보통'} 수준입니다.`);
  }

  // 토익 인사이트
  const avgToeic = calculateAvgToeic(applicants);
  if (avgToeic > 0) {
    insights.push(`평균 토익 점수는 ${Math.round(avgToeic)}점입니다.`);
  }
  ...
  if (avgActivityCount > 0) {
    insights.push(`평균 ${avgActivityCount.toFixed(1)}개의 활동을 자소서에 언급합니다.`);
  }
  ...
  if (teamProjectCount > total * 0.5) {
    insights.push(`${((teamProjectCount / total) * 100).toFixed(0)}%가 팀 프로젝트 경험을 강조합니다.`);
  }
  ...
  if (leadershipCount > total * 0.3) {
    insights.push(`${((leadershipCount / total) * 100).toFixed(0)}%가 리더십 경험을 언급합니다.`);
  }
  ...
  if (awardCount > total * 0.2) {
    insights.push(`${((awardCount / total) * 100).toFixed(0)}%가 수상 경력을 보유하고 있습니다.`);
  }
